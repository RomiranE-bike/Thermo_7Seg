...
void thermo_action(void){
	...
	T1_value =  eeprom_read_word((uint16_t*)500);
	T2_value =  eeprom_read_word((uint16_t*)600);
...
void thermo_setting(void){
	...
	while( button_value == 196 ){ //  thermo_set_SW1 is activated
		...
		_delay_ms(1);
		eeprom_write_word((uint16_t*)500,T1_value); //write memory
		if(T1_value>999){
			T1_value =0;
		}//end of if
	}//end of if
	...
	while( button_value == 122){ //  thermo_set_SW1 is activated
		...
		_delay_ms(1);
		eeprom_write_word((uint16_t*)600,T2_value); //write memory
		if(T2_value>999){
			T2_value =0;
		}//end of if
	}//end of if
