...
//-------------------------------------------------------------------------------------------------------------
EEMEM unsigned char   eeprom_value;
		...
		while(1){
			...
			if(! (button_PIN & 1<<SETUP_SW) ){					
					...
					while(SETUP_flag == 1){
						...
						//display_num(T1_value); //PASS THE VALUE OF 'eeprom_value' FOR DIGIT BREAKUP AND DISPLAY
						//_delay_ms(display_delay);
						//display_char(null,E,E,2);
						//_delay_ms(display_delay);
						//display_num(T2_value); //PASS THE VALUE OF 'eeprom_value' FOR DIGIT BREAKUP AND DISPLAY
						//_delay_ms(display_delay);	
...
void thermo_action(void){
	...
	//display_char(null,L,O,d);
	//_delay_ms(display_delay);
	
	T1_value =  eeprom_read_word((uint16_t*)10);
	T2_value =  eeprom_read_word((uint16_t*)20);
	...
	if(sens_value_1 < T1_value){
		if(sens_value_2 < T1_value){
		   ...
		   if(RL1_flag & RL2_flag){			   
			...
			display_char(null,null,c,1);//sensor value 1
			_delay_ms(display_delay);
			display_char(null,L,1,1);
			_delay_ms(display_delay); 
			action_PORT |= (1<<RL2_BIT);//Relay 2 on to increase temperature
			display_char(null,null,c,2);//sensor value 1
			_delay_ms(display_delay);
			display_char(null,L,2,1);
			_delay_ms(display_delay);  
		    }//end of if
			}//end of if
			}//end of if
...
void thermo_setting(void){
	...
	while( button_value >220  &&  button_value<235   ){ //  thermo_set_SW1 is activated
		...
		_delay_ms(1);
		eeprom_write_word((uint16_t*)10,T1_value); //write memory
		if(T1_value>999){
			T1_value =0;
		}//end of if
	}//end of if
	...
	while( button_value >170  &&  button_value<200){ //  thermo_set_SW1 is activated
		...
		_delay_ms(1);
		eeprom_write_word((uint16_t*)20,T2_value); //write memory
		if(T2_value>999){
			T2_value =0;
		}//end of if
	}//end of if
